...
import {
  ...
  addMonthlyTask,
  deleteMonthlyTask,
  toggleMonthlyTaskCompletion,
} from "../store/monthly-slice.jsx";

import { calculateMonthlyTaskProgress } from "../store/monthly-slice.jsx";
...
function MonthlyTask() {
  ...
  const handleStartDateChange = (e) => {
    ...
    const lastDay = getLastDayOfMonth(startDateObj);
  ...
  const handleAddTask = () => {
    ...
    dispatch(addMonthlyTask(newTask));
    resetForm();
  };
  ...
  const handleDelete = (taskId) => {
    dispatch(deleteMonthlyTask(taskId));
  };

  const toggleCompletion = async (task) => {
    try {
      await dispatch(toggleMonthlyTaskCompletion(task._id)).unwrap();
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      dispatch(calculateMonthlyTaskProgress());
    }, 1000);
    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    dispatch(fetchMonthlyTasks());
  }, [dispatch]);
