import { monthlyTask } from "../models/monthly.model.js";
import { asyncHandler } from "../utils/asyncHandler.js";
import { ApiError } from "../utils/apiError.js";
import { uploadOnCloudinary } from "../utils/uploadOnCloud.js";
import fs from "fs";

const monthlyTaskController = {
  createTask: asyncHandler(async (req, res) => {
    try {
      ...
      const existedData = await monthlyTask.findOne({
  ...
  getAllTasks: asyncHandler(async (req, res) => {
    try {
      ...
      const tasks = await monthlyTask.find({ user: userId });
  ...
  toggleTaskCompletion: asyncHandler(async (req, res) => {
    ...
    const task = await monthlyTask.findOne({ _id: id, user: userId });
  ...
  deleteTask: asyncHandler(async (req, res) => {
    ...
    const deletedTask = await monthlyTask.findOneAndDelete({
      _id: id,
      user: userId,
    });
